...
from visual.graph import *     # import graphing features (which grabs the general visual stuff)
from controls import *     # grabs my controls and NOT the general visual stuff so must come second

# set up the main display window
setrange = 250
scene = display(width=1050, height=550, x=0, y=100, background=color.white, fov=0.001, range=setrange)
scene.exit = False
...
#   clean up the horizontal velocity graph
    ...
    for obj in Xvel.display.objects:
        ...
        del obj
    del Xvel
    Xvel = gdisplay(x=0,y=0, width=262, height=100, background=color.white, xmin=0, xmax=12, ymin=-vmax, ymax=vmax, title = 'V_x(t)')
...
#   clean up the vertical velocity graph
    ...
    for obj in Yvel.display.objects:
        ...
        del obj
    del Yvel
    Yvel = gdisplay(x=263,y=0, width=262, height=100, background=color.white, xmin=0, xmax=12, ymin=-vmax, ymax=vmax, title = 'V_y(t)')
...
#   clean up the horizontal acceleration graph
    ...
    for obj in Xaccel.display.objects:
        ...
        del obj
    del Xaccel
    Xaccel = gdisplay(x=523,y=0, width=262, height=100, background=color.white, xmin=0, xmax=12, ymin=-vmax, ymax=vmax, title = 'A_x(t)')
...
#   clean up the vertical acceleration graph
    ...
    for obj in Yaccel.display.objects:
        ...
        del obj
    del Yaccel
    Yaccel = gdisplay(x=787,y=0, width=262, height=100, background=color.white, xmin=0, xmax=12, ymin=-vmax, ymax=vmax, title = 'A_y(t)')
...
# Parameter settings
s1 = slider(pos=(-83,-2.5,0), width=4, axis=(25,0,0), min=0, max=90, color = color.magenta, action=lambda: setangle(s1))
...
s2 = slider(pos=(-55,-2.5,0), width=4, axis=(25,0,0),  min=0, max=vmax, color=color.blue, action=lambda: setspeed(s2))
s2.value = s2.max/2.0
setspeed(s2)

s3 = slider(pos=(-26,-2.5,0), width=4, axis=(25,0,0),  min=0, max=10, color = (1,0.5,0.2), action=lambda: setconstant(s3))
s3.value = s3.max/2.
setconstant(s3)
...
while 1:
    ...
    floor = box(pos=(0,-0.4*setrange,0), length=2*setrange, height=setrange/30., width=setrange, color=color.green)    # create ground
    ...
    while not(hitnodrag and hitdrag2 and hitair):                # until all three have landed; w/ margin
        ...
        if (Drag2Ball==1):
            if drag2.pos.y > bottom:                                                    # skip if it has already landed
                ...
                drag = vector(-sign(drag2.velocity.x)*drag2.velocity.x**2, -sign(drag2.velocity.y)*drag2.velocity.y**2,0)
