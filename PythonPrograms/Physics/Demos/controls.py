...
# Sliders modified by Lenore Horner 2009 to permit labelling with slider value (after colorsliders.py from the examples)
...
class slider(ctrl):
    def __init__(self, **args):
        ...
        self.indicator = box(display=disp,
                       ...
                       axis=self.axis,
                       size=(2*width,2*width,2*width), color=scolor)
        self.label = label(display=disp, 
                           pos=self.indicator.pos+vector(0,0,2.*width+1), text="%0.2f" % self.__value,
    ...
    def update(self, pos):
        ...
        if val != self.__value:
            ...
            self.__value = val
            self.label.pos = self.indicator.pos
            self.label.text="%0.2f" % val
            self.execute()
