...
# The percentage of outdoor tiles that have additional decoration on them, such as a tree or rock.
...
def main():
    ...
    # Because the Surface object stored in DISPLAYSURF is returned from the pygame.display.set_mode() function, this is
    # the Surface object that is drawn to the actual computer screen when pygame.display.update() is called.
...
def startScreen():
    ...
    # Unfortunately, Pygame's font & text system only shows one line at a time, so we can't use strings 
...
def runLevel(levels, levelNum):
    ...
    while True: # main game loop
        ...
        for event in pygame.event.get(): # event handling loop
            if event.type == QUIT:
                ...
                # Set the camera move mode.
                    ...
                    currentImage = (currentImage + 1) % len(PLAYERIMAGES) # After the last player image, use the first one.
                    print currentImage
                    mapNeedsRedraw = True
        ...
        if playerMoveTo != None and not levelIsComplete:
            # If the player pushed a key to move, make the move (if possible) and push any stars that are pushable.
...
def floodFill(mapObj, x, y, oldCharacter, newCharacter):
    ...
    # In this game, the flood fill algorithm creates the inside/outside floor distinction. This is a "recursive" function.
...
def drawMap(mapObj, gameStateObj, goals):
    ...
    # mapSurf will be the single Surface object that the tiles are drawn on, so that it is easy to position
    # the entire map on the DISPLAYSURF Surface object. First, the width and height must be calculated.
